...
# Obtient le chemin du répertoire de main.py
...
def fonctionnalite_1():
    ...
    La fonction ne retourne rien puisqu'il s'agit ici d'afficher du texte dans la console.
    """
    with open(f"{CHEMIN_MAIN}/README.txt", "r", encoding="utf-8") as fichier:
        print(fichier.read())
...
def fonctionnalite_3(nom_fichier_presidents, noms_presidents):
    ...
    La fonction ne retourne rien puisqu'il s'agit ici d'afficher du texte dans la console.
    """
    if not os.path.isfile(f"{CHEMIN_MAIN}/{nom_fichier_presidents}"):
        open(f"{CHEMIN_MAIN}/{nom_fichier_presidents}", "x", encoding="utf-8")

    # Lit le contenu du fichier "presidents.txt"
    with open(f"{CHEMIN_MAIN}/{nom_fichier_presidents}", "r", encoding="utf-8") as fichier:
    ...
    def modif_prenoms():
        ...
        with open(f"{CHEMIN_MAIN}/{nom_fichier_presidents}", "w", encoding="utf-8") as fichier_pres:
...
def fonctionnalite_10(noms_fichiers, liste_mots_corpus, matrice):
    """
    Écrit une représentation tabulaire de la matrice TF-IDF du corpus dans un fichier texte.

    :param noms_fichiers: Liste des noms de fichiers dans le corpus.
    :param liste_mots_corpus: La liste des mots dans le corpus.
    :param matrice: La matrice TF-IDF du corpus.
    :param nom_fichier_sortie: Nom du fichier dans lequel écrire la table.
    """
    ...
    with open(f"{CHEMIN_MAIN}/matrice_tfidf.txt", "w", encoding="utf-8") as fichier:
            
        fichier.write((mot_plus_long + len("|  |")) * " ")
        for _ in range(len(noms_fichiers) - 1):
            fichier.write((noms_fichiers_plus_long + len(" | ")) * "_")
        fichier.write((noms_fichiers_plus_long + len("  ")) * "_")
        ...
        for nom_fichier in noms_fichiers:
            fichier.write("| " + ajuster_espaces(nom_fichier, noms_fichiers_plus_long))
        fichier.write("|\n")

        def fermer_tableau():
            fichier.write((mot_plus_long + len("  ")) * "_")
            for _ in range(len(noms_fichiers)):
                fichier.write("|__" + noms_fichiers_plus_long * "_")
            fichier.write("|\n")
        ...
        for indice_mot in range(len(liste_mots_corpus)):
            fichier.write("| " + ajuster_espaces(liste_mots_corpus[indice_mot], mot_plus_long))
            for indice_fichier in range(len(noms_fichiers)):
                fichier.write("| " + ajuster_espaces(str(matrice[indice_mot][indice_fichier]), noms_fichiers_plus_long))
            fichier.write("|\n|")
            fermer_tableau()

        print("La matrice a bien été écrite dans le fichier \"matrice_tfidf.txt\"")
...
def main():
    ...
    noms_fichiers = tt_fich.liste_fichiers(f"{CHEMIN_MAIN}/{NOM_REPERTOIRE_DISCOURS}", "txt")
        ...
        if reponse_utilisateur == "1":
            ...
            while demander_deuxieme_numero:
                print("\nPour accéder aux différentes fonctionnalités, veuillez entrer le numéro associé à celles-ci :"
                      ...
                      "10. Afficher la matrice TF-IDF dans un fichier\n"
