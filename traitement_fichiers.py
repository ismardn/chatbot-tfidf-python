...
def creer_fichiers_minuscule(noms_fichiers, nom_repertoire_discours, nom_repertoire_nettoye):
    ...
    for nom_fichier in noms_fichiers:
        with open(f"{CHEMIN_MAIN}/{nom_repertoire_discours}/{nom_fichier}", "r", encoding="utf-8") as fichier_ancien, \
                open(f"{CHEMIN_MAIN}/{nom_repertoire_nettoye}/{nom_fichier}", "w", encoding="utf-8") as fichier_nettoye:
...
def nettoyage_complet_fichiers(nom_repertoire_nettoye, noms_fichiers, nom_repertoire_discours):
    ...
    La fonction ne retourne rien puisqu'il s'agit ici d'un traitement de fichiers.
    """
    if not os.path.exists(f"{CHEMIN_MAIN}/{nom_repertoire_nettoye}"):
        os.mkdir(f"{CHEMIN_MAIN}/{nom_repertoire_nettoye}")  # Création du répertoire pour fichiers nettoyés s'il n'existe pas

    creer_fichiers_minuscule(noms_fichiers, nom_repertoire_discours, nom_repertoire_nettoye)

    for nom_fichier in noms_fichiers:
        with open(f"{CHEMIN_MAIN}/{nom_repertoire_nettoye}/{nom_fichier}", "r", encoding="utf-8") as fichier:
            ancien_contenu = fichier.read()  # On stocke dans une variable l'ensemble du contenu du fichier

        with open(f"{CHEMIN_MAIN}/{nom_repertoire_nettoye}/{nom_fichier}", "w", encoding="utf-8") as fichier:
