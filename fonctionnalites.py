...
Ce fichier contient différentes fonctions pour explorer le corpus, comme retrouver les noms des présidents, identifier les mots les plus fréquents, ou encore repérer ceux utilisés par l'ensemble des présidents.
"""
...
def mot_max_president(liste_mots, noms_fichiers, matrice, nom_president, nom_repertoire):
    ...
    for nom_fichier in noms_fichiers:
        if nom_president in nom_fichier:
            with open(f"{CHEMIN_MAIN}/{nom_repertoire}/{nom_fichier}", "r", encoding="utf-8") as fichier:
